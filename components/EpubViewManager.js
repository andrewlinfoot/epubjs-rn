...
import { StyleSheet, View, ScrollView, Dimensions, InteractionManager, NativeMethodsMixin, NativeModules, UIManager, ActivityIndicator } from 'react-native';
...
class EpubViewManager extends Component {
  ...
  display(section, target) {
    ...
    if (target === section.index || target === section.href) {
      target = false;
    }
    ...
    if (this.fixed && this.spreads && section.index > 0 && section.index % 2 === 0) {
      // render odd page
    ...
    this.setState({
      ...
      let rendered = Promise.all(renderedPromises).then(() => {
  ...
  append(section) {
    ...
    this.setState({
      ...
      if (view) {
        ...
        view.displayed.then(displaying.resolve, displaying.reject
  ...
  visible() {
    ...
    this.state.sections.forEach(section => {
      ...
      if (this.state.horizontal && position.right - this.scrollProperties.offset > container.left && position.left - this.scrollProperties.offset < container.right) {

        visible.push(view);
      } else if (!this.state.horizontal && position.bottom - this.scrollProperties.offset > container.top && position.top - this.scrollProperties.offset < container.bottom) {
  ...
  _measureAndUpdateScrollProps() {
    ...
    // RCTScrollViewManager.calculateChildFrames is not available on
    // every platform
    FPVisibleScrollViewManager && FPVisibleScrollViewManager.calculateChildFrames && FPVisibleScrollViewManager.calculateChildFrames(ReactNative.findNodeHandle(scrollComponent), this._updateVisible.bind(this));
  }
  ...
  */
  ...
  updateLayout(layout) {
    ...
    if (this.state.horizontal) {
      layout.calculate(bounds.width - margin, bounds.height, margin);
    } else {
  ...
  scrollTo(x, y, silent) {
    ...
    if (x === this.scrollProperties.offsetX && y === this.scrollProperties.offsetY) {
      return; // no change
    }
  ...
  render() {
    return <VisibleScrollView ref={SCROLLVIEW_REF} automaticallyAdjustContentInsets={false} horizontal={this.state.horizontal} pagingEnabled={this.state.horizontal ? true : false} stickyHeaderIndices={[]} style={this.state.horizontal ? styles.horzScrollContainer : styles.vertScrollContainer} onContentSizeChange={this._onContentSizeChange.bind(this)} onLayout={this._onLayout.bind(this)} onScroll={this._onScroll.bind(this)} scrollEventThrottle={DEFAULT_SCROLL_CALLBACK_THROTTLE} removeClippedSubviews={this.props.removeClippedSubviews === false ? false : true} scrollsToTop={false} showsHorizontalScrollIndicator={false} showsVerticalScrollIndicator={false}>
        {this.state.sections.map((section, index) => {
        return <EpubView ref={`section_${section.index}`} key={`scrollview_section:${section.index}`} section={section} horizontal={this.state.horizontal} onPress={this.props.onPress} onLongPress={this.props.onLongPress} format={this.state.layout.format.bind(this.state.layout)} layout={this.state.layout.props} delta={this.state.layout.delta} columnWidth={this.state.layout.columnWidth} spreads={this.state.layout.divisor > 1} gap={this.state.horizontal ? this.state.layout.gap : this.minGap} afterLoad={this._afterLoad.bind(this)} onResize={e => this._onResize(section, e)} onExpanded={e => this._onExpanded(section, e)}
        // willResize={(e)=> this._willResize(section, e)}
        boundsHeight={this.props.bounds.height || this._bounds.height} boundsWidth={this.props.bounds.width || this._bounds.width} request={this.props.request} baseUrl={this.props.baseUrl} origin={this.props.origin} backgroundColor={this.props.backgroundColor} lastSectionIndex={this.props.lastSectionIndex} onLayout={l => {
          this._onChildLayout(index, l);
        }} />;
      })}
      </VisibleScrollView>;
  }
